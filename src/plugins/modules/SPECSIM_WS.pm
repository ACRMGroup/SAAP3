...
# Specsim web service
my $specsimURL = "http://www.bioinf.org.uk/cgi-bin/specsim/query.pl?";
...
#-------------------------------------------------------------------------
# This routine replaces Lisa's database access to obtain species simlarities.
# It uses a file dumped from the database to create a DBM hash. If that is
# present and up-to-date, then values are just grabbed from there. If not
# then it is re-created from the database dump file
sub GetSpecsim
{
    my ($specsimDumpFile, $specsimHashFile, $species1, $species2) = @_;

    my $url = $specsimURL . "species1=$species1&species2=$species2";
