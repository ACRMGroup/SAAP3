...
foreach my $jsonSaaps (@jsonSaaps)
{
    ...
    print "$Binding,$SProtFT,$Interface,$Relaccess,$Impact,$HBonds,$SPhobic,$CPhilic,$BCharge,$SSGeom,$Voids,$MLargest,$NLargest,$Clash,$Glycine,$Proline,$CisPro,$Phenotype\n";    
...
# The json files generated by runPipelineModels.pl have invalid characters
# in the value for structure type that cause json->relaxed->decode to
# return an error. This function removes those invalid characters.
{
    my $cleanedContent = "";
    my ($inputContent) = @_;
    my @inputLines = split(/\n/, $inputContent);
    ...
    {
        if ($inputLine =~ /structuretype/)
        {
            $cleanedContent .= '     "structuretype": "model",' . "\n";
        }
        else
        {
            $cleanedContent .= "$inputLine\n";
        }
    }
