...
$::printjson = defined($::printall)?1:0;
...
#-- Define files
#-- all the files can be re-directed to /tmp by changing the directories in configPred file 
my $jsonFile;
if(!defined($::json))
{
...
if(!($::json))  
{
    print STDERR "     ... Run the pipline" if($::v);
    my $runPipline = `$configPred::perl $config::saapUniprotPipeline -limit=$::pdblimit $sprot $nat $pos $mut > $jsonFile`;
    system( $runPipline );
    print STDERR "          ... Done\n" if($::v);
}
...
my (%allPrediction) = runPredectour($::mlimit);
print STDERR "        ... Done\n" if($::v);

#-------------------------------------------------------------------------#
#------------------------  Averaging result  -----------------------------#
#-------------------------------------------------------------------------#
print STDERR "     ... Avraging          " if($::v);
my ($avgPred,$avgConf,%pdbPredection) = averag(%allPrediction);
print STDERR "       ... Done\n" if($::v); 
...
sub saveIDS
{
    ...
    if(open(FILE, $file))
    {
        while(<FILE>)
        {
            ...
            #-- (num:uniprotac:res:nat:mut:pdbcode:chain:resnum:mutation:structuretype:resolution:rfactor)
            #-- change in this subroutine in case you have different Id combination or more than one Id
    ...
    {     
        ...
        { 
            ...
            { 
                ...
                {
                    #--- prediction result (PD or SNP) will be saved as $p{"pred".$i}[$instance] 
                    #    for line no.$instance using model no. $i , each line ($instance) 
                    #    represent a different combination of PDB/Chain we are testing with
                    $prediction{'pred'.$instance}[$i] = $1;
                    
                    #--- prediction confidante result (i.e 0.832) will be saved as 
                    #     $pred{"conf".$i}[$instance] for line no.$instance using model no. $i
...
sub averag
{
    ...
    {    
        ...
        #  $avgPdbPred and $avgPdbConf are the average of different Models results for $i PDB entry
...
sub printPred
{
    ...
    if($::printpdb || $::printall)
    {
        ...
        for(my $i=1; $i<$::pdblimit+1; $i++)
        {   
            #-- To print all the result including each PDB/Chain combination and the different Models prediction
            if($::printall)
            {
...
#{ 
           # print JSON <<__EOF;
    #    \{"Prediction": "$allPrediction{'pred'.$i}[$e]", "Confidence": "$allPrediction{'conf'.$i}#[$e]"\}, 
#__EOF
 #           push @results, '{"Prediction": "$allPrediction{"pred".$i}[$e]", "Confidence": "$allPrediction{"conf".$i}[$e]"}';
  #      }
    


        for(my $e=1; $e<$::mlimit+1; $e++)
#(my $i=0; $i<int(@results); $i++)
        {
            
            print "{\"Prediction\": \"$allPrediction{'pred'.$i}[$e]\", \"Confidence\": \"$allPrediction{'conf'.$i}[$e]\"}";
            if($i<($::mlimit-1))
            {
                print ",\n";
            }
...
@results\}\],
                       "Avgerge": \{"Prediction": "$pdbPredection{pred}[$i]", "Confidence": "$pdbPredection{conf}[$i]"\} 
                           \}  
                 \}\],
__EOF
        }  
...
print JSON <<__EOF;
        \{"FinalPrediction": "$avgPred", "FinalConfidence": "$avgConf"\}
